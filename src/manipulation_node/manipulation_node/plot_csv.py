...
parser = argparse.ArgumentParser(description='Plot CSV data for pose analysis.')
parser.add_argument('csv_file', nargs='?', default='random_pose.csv', help='Path to the CSV file')
...
with open("random_pose_cpu.csv", 'r') as f:
    reader = csv.reader(f)
    next(reader)
    for row in reader:
...
cpu_vals = np.array(cpu_vals) / psutil.cpu_count()  # Normalise to per core percentage
